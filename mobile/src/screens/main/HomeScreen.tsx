    ...
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} showsVerticalScrollIndicator={false}>
      ...
      <View style={[s.section, { backgroundColor: sectionBg, paddingBottom: 32 }]}>
        ...
        <ScrollView
          ...
          style={{ marginTop: 16 }}
          contentContainerStyle={s.ageScroll}
        >
          {AGE_GROUPS.map((g) => (
            <TouchableOpacity
              key={g.label}
      ...
      <View style={[s.section, { backgroundColor: colors.bg }]}>
        <Text style={[s.sectionTitle, { color: colors.text }]}>Recent Listings</Text>
        {loading ? (
          <ActivityIndicator color={PRIMARY} style={{ marginTop: 20 }} />
        ) : recentItems.length === 0 ? (
          <Text style={[s.emptyText, { color: colors.text2 }]}>No listings yet</Text>
        ) : (
          <View style={s.itemsGrid}>
            {recentItems.map((item, index) => (
              <View key={item.id} style={index % 2 === 0 ? s.cardLeft : s.cardRight}>
                <ItemCard item={item} onPress={handleItemPress} />
              </View>
            ))}
          </View>
        )}
        <TouchableOpacity style={s.viewAllBtn} onPress={() => navigation.navigate('Browse' as any)}>
...
const s = StyleSheet.create({
  ...
  ageCard: { width: 88, borderRadius: 16, padding: 12, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.06, shadowOffset: { width: 0, height: 2 }, shadowRadius: 6, elevation: 2, borderWidth: 1 },
  ageIcon: { width: 48, height: 48, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  ageEmoji: { fontSize: 22 },
  ageLabel: { fontSize: 12, fontWeight: '700', textAlign: 'center' },
  ageRange: { fontSize: 10, marginTop: 2, textAlign: 'center' },
  ...
  viewAllBtn: { marginTop: 12, alignSelf: 'center', paddingHorizontal: 24, paddingVertical: 10, borderRadius: 99, borderWidth: 2, borderColor: PRIMARY },
  ...
  supportBody: { fontSize: 13, textAlign: 'center', lineHeight: 20, maxWidth: 300, marginBottom: 4 },
});
