...
function prStatusColor(s: number): string {
  if (s === PurchaseRequestStatus.Accepted) return '#16a34a';
  if (s === PurchaseRequestStatus.Declined || s === PurchaseRequestStatus.Cancelled) return '#dc2626';
  if (s === PurchaseRequestStatus.Completed) return '#2563eb';
  return '#d97706';
}
...
export default function DashboardScreen({ navigation }: Props) {
  ...
  const refreshControl = (
    <RefreshControl refreshing={loading} onRefresh={refresh} tintColor="#e91e8c" />
  );

  const renderContent = () => {
    if (loading && !data.myRequests.length && !data.incomingRequests.length &&
        !data.shipments.length && !data.ebills.length) {
      return <ActivityIndicator color="#e91e8c" style={{ marginTop: 40 }} />;
    }
...
const styles = StyleSheet.create({
  ...
  tabBar: { backgroundColor: '#fff', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#f0f0f0', flexGrow: 0 },
  ...
  tabItemActive: { backgroundColor: '#fce7f3' },
  tabLabel: { fontSize: 14, color: '#888', fontWeight: '500' },
  tabLabelActive: { color: '#e91e8c', fontWeight: '700' },
  ...
  btnDecline: {
    flex: 1, height: 40, borderRadius: 10,
    borderWidth: 1.5, borderColor: '#dc2626', alignItems: 'center', justifyContent: 'center',
  },
  btnDeclineText: { color: '#dc2626', fontSize: 14, fontWeight: '600' },
  btnDisabled: { opacity: 0.5 },

  btnPrimary: {
    height: 42, borderRadius: 10,
    backgroundColor: '#e91e8c', alignItems: 'center', justifyContent: 'center', marginTop: 4,
  },
  btnPrimaryText: { color: '#fff', fontSize: 14, fontWeight: '600' },

  btnSecondary: {
    height: 40, borderRadius: 10, borderWidth: 1.5,
    borderColor: '#e91e8c', alignItems: 'center', justifyContent: 'center', marginTop: 4,
  },
  btnSecondaryText: { color: '#e91e8c', fontSize: 14, fontWeight: '600' },

  confirmedRow: { marginTop: 4 },
  confirmedText: { fontSize: 13, color: '#16a34a', fontWeight: '600' },
