...
| `FRONTEND_URL` | `http://localhost` | Frontend URL for CORS (must be HTTPS in production) |
| `ALLOWED_HOSTS` | `localhost` | Semicolon-separated ASP.NET Core `AllowedHosts` (e.g. `mamvibe.com;api.mamvibe.com`) |
| `REDIS_URL` | (unset) | Redis connection string — enables distributed cache + SignalR backplane; falls back to in-memory when unset |
| `ANTHROPIC_API_KEY` | (unset) | Claude AI API key for AI assistant feature |
| `PIGEONEXPRESS_API_KEY` | (unset) | Pigeon Express courier API key |

## Secrets Management (Doppler)

MamVibe supports [Doppler](https://www.doppler.com/) as an alternative to maintaining a local `.env` file. Once the CLI is installed and authenticated, run:

```bash
doppler run -- docker compose up --build
```

Doppler injects all secrets at process startup, so no `.env` file is needed on CI or production hosts. The `doppler.yaml` in the repo root maps the project to the `mamvibe/prd` config automatically.

For local development without Doppler, `cp .env.example .env` and fill in the values as before.
...
|-------|-------------|---------|
| **Database** | Composite indexes | `AuditLog(UserId+CreatedAt, Action)`, `ItemModerationLog(ItemId+CreatedAt, AdminId)`, `Feedback(CreatedAt)`, `DoctorReview(IsApproved, CreatedAt)`, `ChildFriendlyPlace(IsApproved, CreatedAt, AgeToMonths)` |
| **Output Cache** | HTTP response caching | 30 s base policy; `Categories` endpoint cached for 1 h with cache tags for targeted invalidation |
| **Distributed Cache** | Redis (`IDistributedCache`) | Replaced `IMemoryCache` in `BlockedUserMiddleware` and `AdminService` so block/unblock state is consistent across multiple API replicas |
| **SignalR** | Redis backplane | `AddStackExchangeRedis` backplane enables horizontal scaling — messages from any API instance reach all connected clients |
| **Connection pooling** | PgBouncer | Transaction-mode pooler sits between the API and PostgreSQL: 500 max clients, pool size 20 per database, dramatically reducing connection overhead under load |
...
| # | Rule | Implementation |
|---|------|---------------|
| 1 | Enforce HTTPS | `UseHttpsRedirection` (production only via `IsProduction()`) + `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload` |
...
| 7 | CORS trusted origins | Fail-fast on missing/non-HTTPS `FrontendUrl`; `WithOrigins(...)`, explicit headers and methods |
| 8 | Suppress error details | Generic messages in production; stack traces never exposed; API error messages capped at 200 chars in frontend |
| 9 | Rate limiting | Global (200/min), Auth (30/min), Upload (20/min), EBill resend (3/min) + Identity lockout |
| 10 | No sensitive data in logs | Logs contain only IDs and status codes — no passwords, tokens, or PII; `console.warn/error` stripped from frontend production builds |
...
| Hardening | Details |
|-----------|---------|
| **AllowedHosts lockdown** | `appsettings.json` defaults to `localhost`; production value injected via `ALLOWED_HOSTS` env var to prevent Host Header Injection |
| **Metrics protection** | `MetricsProtectionMiddleware` returns 404 for `/metrics` from non-internal IPs; IPv4-mapped IPv6 (`::ffff:x.x.x.x`) unwrapped for correct Docker network matching |
| **DataProtection persistence** | Keys written to `/app/dataprotection-keys` volume; container restarts no longer invalidate user sessions |
| **Resource limits** | API: 2 CPU / 1 GB; PostgreSQL: 1 CPU / 512 MB — prevents a runaway service from taking down the host |
| **Monitoring ports hidden** | `docker-compose.prod.yml` removes external port bindings for Prometheus, Grafana, and Loki |
| **Log retention** | Loki compactor enforces 31-day retention to limit disk growth |
