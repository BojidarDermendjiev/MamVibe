...
public class UsersPublicTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task UpdateProfile_WithoutAuth_Returns401()
    {
        ...
        var response = await _client.PutAsJsonAsync("/api/users/profile", dto);
    ...
    {
        var response = await _client.GetAsync("/api/users/dashboard/items");
        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
    ...
    {
        var response = await _client.GetAsync("/api/users/dashboard/liked");
        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
    ...
    public async Task RegisterPushToken_WithoutAuth_Returns401()
    {
        ...
        var response = await _client.PostAsJsonAsync("/api/users/push-token", dto);
        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
}
...
public class UsersAuthTests : IClassFixture<GeneralAuthWebApplicationFactory>
{
    ...
    {
        var response = await _client.GetAsync($"/api/users/{GeneralAuthWebApplicationFactory.TestUserId}");
