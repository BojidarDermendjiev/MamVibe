  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        {/* ── Header ── */}
        <div className="relative bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-100 px-6 py-5">
          <button
            onClick={onCancel}
            className="absolute top-4 right-4 p-1 rounded-full text-gray-400 hover:text-gray-600 hover:bg-amber-100/60 transition-colors"
          >
            <X size={16} />
          </button>
          <div className="flex items-start gap-3 pr-6">
            ...
            <div>
              <h2 className="font-semibold text-gray-800 text-base">Buyer Alert</h2>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {/* Buyer info pill */}
          <div className="flex items-center gap-3 p-3 bg-peach-light/30 rounded-xl border border-lavender/20">
            <Avatar src={buyerAvatarUrl} size="md" />
            <div className="min-w-0">
              <p className="font-medium text-primary truncate">{buyerName ?? 'Unknown Buyer'}</p>
              <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full mt-0.5">
